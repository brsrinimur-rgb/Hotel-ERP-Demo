...
page = st.sidebar.radio("Navigation", visible_pages, format_func=lambda p: f"{MODULE_ICONS.get(p, '•')}  {p}", key="nav_page")
...
# ---------------- Dashboard ----------------
    ...
    maintenance_rooms = scalar("SELECT COUNT(*) FROM rooms WHERE status='Maintenance'")
    ...
    st.markdown(
        f"<div class='dashboard-greeting'>Good day, {user['full_name']}</div>"
        f"<div class='dashboard-subcopy'>Live overview of rooms, revenue, service, inventory and approvals.</div>",
        unsafe_allow_html=True
    )

    k1,k2,k3,k4 = st.columns(4)
    k1.metric("Occupancy", f"{occupancy:.1f}%", f"{occupied} of {total_rooms} rooms")
    k2.metric("Today's Arrivals", int(checkins_today), f"{checkouts_today} departures")
    k3.metric("Room Revenue", money(room_revenue), f"ADR {money(avg_daily_rate)}")
    k4.metric("Restaurant Sales", money(restaurant_sales), f"{int(open_kot)} open KOT")

    k5,k6,k7,k8 = st.columns(4)
    k5.metric("Net Cash", money(float(collections or 0)-float(expenses or 0)))
    k6.metric("Outstanding Bills", money(outstanding_bills))
    k7.metric("Open Approvals", int(pending_approvals))
    k8.metric("In-house Guests", int(inhouse_guests), f"RevPAR {money(revpar)}")
    ...
    qa2.button("🏨 Check In/Out", use_container_width=True, on_click=_go, args=("Front Desk",))
    ...
    qa6.button("💳 Receive Payment", use_container_width=True, on_click=_go, args=("Payments",))
    ...
    with left:
        ...
        room_trend = query("""
            SELECT checkin_date AS day,
                   SUM(MAX(julianday(checkout_date)-julianday(checkin_date),1)*room_rate) AS room_revenue
            FROM reservations
            WHERE checkin_date >= date('now','-29 day')
            GROUP BY checkin_date
            ORDER BY checkin_date
        """)
        restaurant_trend = query("""
            SELECT substr(order_time,1,10) AS day, SUM(amount) AS restaurant_sales
            FROM orders
            WHERE substr(order_time,1,10) >= date('now','-29 day')
            GROUP BY substr(order_time,1,10)
            ORDER BY day
        """)
        trend = pd.DataFrame({"day": pd.date_range(end=date.today(), periods=30).strftime("%Y-%m-%d")})
        trend = trend.merge(room_trend, on="day", how="left").merge(restaurant_trend, on="day", how="left")
        trend[["room_revenue","restaurant_sales"]] = trend[["room_revenue","restaurant_sales"]].fillna(0)
        st.line_chart(trend.set_index("day"), height=300)
        ...
        if not room_status.empty:
            st.bar_chart(room_status.set_index("status"), height=250)

    with right:
        st.subheader("Room Availability")
        st.markdown(
            f"""<div class='status-grid'>
              <div class='status-tile'><div class='status-name'><span class='status-dot dot-green'></span>Available</div><div class='status-value'>{int(available_rooms)}</div></div>
              <div class='status-tile'><div class='status-name'><span class='status-dot dot-blue'></span>Occupied</div><div class='status-value'>{int(occupied)}</div></div>
              <div class='status-tile'><div class='status-name'><span class='status-dot dot-orange'></span>Reserved</div><div class='status-value'>{int(reserved_rooms)}</div></div>
              <div class='status-tile'><div class='status-name'><span class='status-dot dot-red'></span>Maintenance</div><div class='status-value'>{int(maintenance_rooms)}</div></div>
            </div>""", unsafe_allow_html=True
        )

        st.subheader("Live Alerts")
        stocks = stock_df()
        stock_alerts = stocks[stocks["stock_status"]=="REORDER"] if not stocks.empty else stocks
        if open_maintenance:
            st.markdown(f"<div class='alert-card red'><div class='alert-title'>🛠️ Maintenance attention</div><div class='alert-copy'>{int(open_maintenance)} ticket(s) remain open.</div></div>", unsafe_allow_html=True)
        if delayed_kitchen:
            st.markdown(f"<div class='alert-card'><div class='alert-title'>⏱️ Delayed kitchen orders</div><div class='alert-copy'>{int(delayed_kitchen)} order(s) are beyond target preparation time.</div></div>", unsafe_allow_html=True)
        if pending_approvals:
            st.markdown(f"<div class='alert-card blue'><div class='alert-title'>✅ Approval queue</div><div class='alert-copy'>{int(pending_approvals)} request(s) are waiting for action.</div></div>", unsafe_allow_html=True)
        if stock_alerts.empty:
            st.markdown("<div class='alert-card green'><div class='alert-title'>📦 Stock healthy</div><div class='alert-copy'>No inventory item is below reorder level.</div></div>", unsafe_allow_html=True)
        else:
            st.markdown(f"<div class='alert-card red'><div class='alert-title'>📦 Low-stock warning</div><div class='alert-copy'>{len(stock_alerts)} item(s) are below reorder level.</div></div>", unsafe_allow_html=True)
            st.dataframe(stock_alerts[["item_code","item_name","available_qty","reorder_level","unit"]], use_container_width=True, hide_index=True, height=210)
    ...
    room_table = query("SELECT room_no,room_type,status,housekeeping,rate FROM rooms ORDER BY room_no")
    o1,o2 = st.columns([1,1.45])
    with o1:
        st.markdown("#### Payment Mix")
        if payment_mix.empty:
            st.info("No payment transactions recorded yet.")
        else:
            st.bar_chart(payment_mix.set_index("payment_method"), height=260)
    with o2:
        st.markdown("#### Room Operations")
        st.dataframe(room_table, use_container_width=True, hide_index=True, height=295)
