...
def _normalize_anb_pos_terminal_id(v):
    """
    V29 fix: traf_09582037.xlsx (ANB POS terminal export) carries Terminal ID
    as the 8-digit ANB terminal code with a constant 8-digit account/merchant
    suffix appended ("01300000"), e.g. "5561069001300000" for terminal
    "55610690". ANB's own bank statement narration only ever uses the bare
    8-digit code, so the padded form can never match during settlement.
    Confirmed on real July 2026 data: 95 of 802 real card settlement batches
    (SAR 469,033) carried this exact 16-digit/suffix shape, all traced to
    this one file's terminal_id column. Strip the known suffix back to the
    canonical 8-digit code; any other shape passes through unchanged so
    this never touches terminal IDs from other files/formats.
    """
    s=str(v).strip()
